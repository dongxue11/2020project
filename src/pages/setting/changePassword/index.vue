  ...
  methods: {
    makeSureChange(){
      if(this.password===this.rePassword){
        post(`/aplt/modifyLoginPassword?phone=${this.phoneNumber}&code=${this.verificationCode}&type=2_0&newPassword=${this.password}&confirmPassword=${this.rePassword}`).then(res=>{
          if(res.code===0){
            Dialog.alert({
